...
def calc_iou(pred_boxes, true_boxes):
    # 计算IOU
    pred_boxes = np.expand_dims(pred_boxes, -2)  # [N, 1, 4]
    true_boxes = np.expand_dims(true_boxes, 0)  # [1, V, 4]
    # [N, 1, 2] & [1, V, 2] ==> [N, V, 2]
    intersect_mins = np.maximum(pred_boxes[..., :2], true_boxes[..., :2])
    intersect_maxs = np.minimum(pred_boxes[..., 2:], true_boxes[..., 2:])
    intersect_wh = np.maximum(intersect_maxs - intersect_mins, 0.)
    intersect_area = intersect_wh[..., 0] * intersect_wh[..., 1]  # 大小: [N, V]
    pred_box_wh = pred_boxes[..., 2:] - pred_boxes[..., :2]  # 大小: [N, 1, 2]
    pred_box_area = pred_box_wh[..., 0] * pred_box_wh[..., 1]  # 大小: [N, 1]
    true_boxes_wh = true_boxes[..., 2:] - true_boxes[..., :2]  # [1, V, 2]
    true_boxes_area = true_boxes_wh[..., 0] * true_boxes_wh[..., 1]  # [1, V]
    iou = intersect_area / (pred_box_area + true_boxes_area - intersect_area + 1e-10)  # 大小: [N, V]
    return iou
...
def evaluate_on_cpu(y_pred, y_true, num_classes, calc_now=True, score_thresh=0.5, iou_thresh=0.5):
    ...
    #           [None, 26, 26, 255],
    #           [None, 52, 52, 255],
    num_images = y_true[0].shape[0]  # 检测依赖的layer数
    ...
    for i in range(num_images):
        ...
        for j in range(3):
            # 3个feature map
            true_probs_temp = y_true[j][i][..., 5:]  # 大小: [13, 13, 3, 80]
            true_boxes_temp = y_true[j][i][..., 0:4]  # 大小: [13, 13, 3, 4] (x_center, y_center, w, h)
            object_mask = true_probs_temp.sum(axis=-1) > 0  # [13, 13, 3]
            true_probs_temp = true_probs_temp[object_mask]  # [V, 3] V: 当前图片有目标的box
            true_boxes_temp = true_boxes_temp[object_mask]  # [V, 4]
            true_labels_list += np.argmax(true_probs_temp, axis=-1).tolist() # [V], 类别
            true_boxes_list += true_boxes_temp.tolist()  # [V, 4] (x_center, y_center, w, h)
        if len(true_labels_list) != 0:
            for cls, count in Counter(true_labels_list).items():
                true_labels_dict[cls] += count
        true_boxes = np.array(true_boxes_list)  # [V, 4] (xmin, ymin, xmax, ymax)
        ...
        if pred_labels_list == []:
            continue
        iou_matrix = calc_iou(pred_boxes, true_boxes)  # calc iou [N, V]
        max_iou_idx = np.argmax(iou_matrix, axis=-1)  # [N]
        correct_idx = []
        correct_conf = []
    ...
    if calc_now:
        recall = sum(true_positive_dict.values()) / (sum(true_labels_dict.values()) + 1e-6)  # 防止分母为0
...
def evaluate_on_gpu(sess, gpu_nms_op, pred_boxes_flag, pred_scores_flag, y_pred, y_true, num_classes, calc_now=True, score_thresh=0.5, iou_thresh=0.5):
    ...
    for i in range(num_images):
        ...
        for j in range(3):
            ...
            true_boxes_temp = y_true[j][i][..., 0:4]  # 大小: [13, 13, 3, 4] (x_center, y_center, w, h)
            object_mask = true_probs_temp.sum(axis=-1) > 0  # [13, 13, 3]
            true_probs_temp = true_probs_temp[object_mask]  # [V, 3] V: 当前图片真实类的个数
            true_boxes_temp = true_boxes_temp[object_mask]  # [V, 4]
            true_labels_list += np.argmax(true_probs_temp, axis=-1).tolist() # [V], labels
            true_boxes_list += true_boxes_temp.tolist()# [V, 4] (x_center, y_center, w, h)
        if len(true_labels_list) != 0:
            for cls, count in Counter(true_labels_list).items():
                true_labels_dict[cls] += count
