...
parser.add_argument("--use_warm_up", type=lambda x: (str(x).lower() == 'true'), default=False,
                    help="Whether to use warm up strategy.")  # warm up strategy
...
logging.basicConfig(level=logging.DEBUG, format='%(asctime)s %(levelname)s %(message)s',
                    datefmt='%a, %d %b %Y %H:%M:%S', filename=args.progress_log_path, filemode='w')  # 设置日志
...
#模型定义
yolo_model = yolov3(args.class_num, args.anchors)  # yolo-v3
    ...
    for epoch in range(args.total_epoches):
        for i in range(args.train_batch_num):
            ...
            if global_step_ % args.val_evaluation_freq == 0 and global_step_ > 0:
                ...
                recall = float(sum(true_positive_dict.values())) / (sum(true_labels_dict.values()) + 1e-6)  # avoid divided by 0
        ...
        shuffle_and_overwrite(args.train_file) # manually shuffle the training data in a new epoch
